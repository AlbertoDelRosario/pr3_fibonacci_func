...
is
   ...
   with
       ...
       Post => 
         ...
         and then fibo_closest_value'Result >= 1;  
     
   function fibo_values_vector (value : Positive) return arrayOfInts
   --Devuelve un vector con la sucesión de fibonacci extendida hasta "value"
   with 
       Pre =>
	 value <= 46,
       Post =>
         (for all J in fibo_values_vector'Result'Range =>
	    fibo_values_vector'Result(J) = fibo_get_value(J));
